...
import sys
sys.path.append('/home/charleshen/project/wanglaoshi/optimal_depth_features/script')
...
def tree_im(df, n_jobs= -1):
    ...
    '''
    #get best paras
    optimal_paras, _ = get_optimal_paras.get_tree_paras(df,  
                                                      method = 'both', 
                                                      n_iter = 10, 
                                                      cv = 5, 
                                                      n_jobs = n_jobs,
    ...
    tree_ims = get_optimal_paras.tree_features(df, optimal_paras)
...
def SFE(df, estimator, param_grid, im_method = 'seq', sel_method = 'sfe',
    ...
    if sel_method == 'sfe':
        ...
        for i in range(myloop):
            ...
            task_zip = zip(Xs,repeat(y),repeat(estimator),repeat(best_paras))
    ...
    if sel_method == 'ofe':
        ...
        task_zip = zip(Xs,repeat(y),repeat(estimator),
                       repeat(param_grid),repeat(1),
                       repeat(3),repeat(5),repeat(True))
...
if __name__ == "__main__":                                        
    ...
    df = pd.read_csv('../data/bairong_valid.csv',index_col = 'uid')
    ...
    #estimator = tree.DecisionTreeClassifier(max_leaf_nodes = 10,min_samples_leaf = 5)
    #param_grid = {'max_depth': randint(5,20)}
   
    results = SFE(df, estimator, param_grid, im_method = 'lasso', sel_method = 'ofe',
